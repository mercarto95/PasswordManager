...
class Password:
    ...
    def generateSymbols(self, numOfChars):
        ...
        for i in range(numOfChars):
            if(i %2 == 0):
                string += chr(random.randint(35, 38))
            else:
    ...
    def view(self, db, target = "", all=False):
        if(all == False):
            ...
            answer = db.execute(f"SELECT * FROM passwords;")
            db.commit()
            print( "Selection all : " , answer)
            myList = []
            for row in answer:
                #print("=="*20)
                myList.append(row[0])
                print(row[0])
            return myList
    def deleteAllPasswords(self, db):
        db.execute("DELETE FROM passwords; ")
        db.commit()
        
    def checkAuthentification(self, email, password, db):
        answer =db.execute (f"SELECT password FROM user WHERE email = '{email}' ;")
        dbPassword = ""
        for row in answer:
            print("#################################################", row[0])
            dbPassword = row[0]
    ...
    def storeToDB(self, user, autonotification, passwordFor, password, db):
        if( not self.isExisting(db, passwordFor)  ):
            ps = 95
            #msg = "INSERT INTO passwords VALUES ( '" + passwordFor + "', '" + password + "', " + str(ps) + ");"
            #print(msg)
